  ...
  return (
    <AppShell>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold">{clientDisplayName(c)}</h1>
          <p className="text-sm text-slate-500">
            {[c.name !== clientDisplayName(c) ? c.name : null, c.city, c.province].filter(Boolean).join(" · ") || "—"}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <span className={`rounded-full px-3 py-1 text-sm ${c.is_active ? "bg-emerald-100 text-emerald-800" : "bg-slate-200 text-slate-600"}`}>
            {c.is_active ? "Active" : "Archived"}
          </span>
          <Link href={`/clients/${c.id}/edit`} className="rounded-md bg-slate-100 px-4 py-2 text-sm">
            Edit
          </Link>
          <button
            className={`rounded-md px-4 py-2 text-sm ${c.is_active ? "bg-amber-50 text-amber-700" : "bg-emerald-50 text-emerald-700"}`}
      ...
      <div className="mb-6 grid gap-4 rounded-xl bg-white border border-black/[0.06] p-5 shadow-[0_1px_2px_rgba(0,0,0,0.03),0_12px_28px_-12px_rgba(0,0,0,0.14)] md:grid-cols-3">
        <div>
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <Link href={`/invoices/create?client=${c.id}`} className="rounded-md bg-win-600 px-4 py-2 text-sm text-white">
            ...
            <tbody>
              {data.recent_invoices.map((inv) => (
                <tr key={inv.id} className="border-t">
                  <td className="py-2">
                    <Link href={`/invoices/${inv.id}`} className="text-win-600">
